    ...
    <span>
      ...
      <ClientOnly>
        <span class="text-xs op-50" :title="toDate(commit.date_timestamp).toString()">
          {{ t('committedOn', { props: { date: toDate(commit.date_timestamp as unknown as Date).toLocaleDateString() }, omitEmpty: true }) || t('changelog.committedOn', { props: { date: toDate(commit.date_timestamp as unknown as Date).toLocaleDateString() } }) }}
        </span>
      </ClientOnly>
    </span>
  </div>
