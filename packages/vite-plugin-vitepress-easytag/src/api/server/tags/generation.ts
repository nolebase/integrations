...
import type { Doc, Metadata } from '@nolebase/shared/types'
...
function checkTagsForPageCanBeGenerated(metadata: Metadata, filePath: string): {
  reason?: string;
  canBeGenerated: boolean;
  content?: matter.GrayMatterFile<string>
} {
  try {
    const found = metadata.docs.find((item) => {
...
export const handleGetTagsGeneration = (handlerDependencies: HandlerDependencies): RequestHandler => {
  return async (req, res) => {
    try {
      ...
      const canBeGenerated = checkTagsForPageCanBeGenerated(handlerDependencies.metadata, filePath)
...
export const handlePostTagsGeneration = (handlerDependencies: HandlerDependencies): RequestHandler => {
  return async (req, res) => {
    try {
